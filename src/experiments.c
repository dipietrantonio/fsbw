...
float timed_posix_read_write(int block_index, ProgramOptions *opts, short is_read){
    ...
    // TODO: bring this outside this function, we do not want to allocate/deallocate memory all the time.    
...
float run_experiment(ProgramOptions *opts){
    ...
    for(int count = 0; count < opts->block_count; count++){
        ...
        if(((float)rand())/RAND_MAX <= opts->read_prob){
            // it is a read op
            read_bw[num_reads++] =  block_size_in_mb/timed_posix_read_write(current_block, opts, READ);
        }else{
            // it is a write op
            write_bw[num_writes++] = block_size_in_mb/timed_posix_read_write(current_block, opts, WRITE);
        }
    }
