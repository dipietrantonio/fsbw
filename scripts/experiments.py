...
def big_file_experiment():
    ...
    # experiment set 1 - bandwidth as a function of file size.
    inputs = ['test_1.bin', ] #'test_2.bin', 'test_3.bin', 'test_4.bin']
    file_size = ['1024G']  #['512M', '4G', '20G', '200G']
    file_to_filesize = dict(zip(inputs, file_size))
    block_size = ['16M']
    # block_count = [5000]
    read_prob = [1]
    experiments = list(product(inputs, block_size, read_prob))
    ...
    for exp in experiments:
        for l, lyt in enumerate(lyts):
            ...
            with open(filename, "w") as fp:
                fp.truncate(parse_size(filesize))
            print(block_size)
            res = exec_fsbw(filename=exp[0], block_size=exp[1], read_prob=exp[2])
            all_results.append((exp, lyt, res))
    with open("big_file_experiment.bin", "wb") as fp:
        pickle.dump(all_results, fp)
